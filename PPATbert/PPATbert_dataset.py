...
from PPATbert_graph_build import graph_builder

stack_spliter = None


class PPATbert_dataset(Dataset):
...
def get_dataloader(config, path, tokenizer, logger, mode):
    return DataLoader(dataset=PPATbert_dataset(config, path, tokenizer, logger, mode),
                      batch_size=1,
                      shuffle=('train' in mode),
                      collate_fn=PPATbert_dataset.collate_fn)
