...
from PPATlong_graph_build import graph_builder

stack_spliter = None


class PPATlong_dataset(Dataset):
...
def get_dataloader(config, path, tokenizer, logger, mode):
    return DataLoader(dataset=PPATlong_dataset(config, path, tokenizer, logger, mode),
                      batch_size=1,
                      shuffle=('train' in mode),
                      collate_fn=PPATlong_dataset.collate_fn)
