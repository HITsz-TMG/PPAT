...
def option():
    ...
    def _model_config():
        ...
        parser.add_argument('--gat_num_heads', default=12, type=int, help='')
        parser.add_argument('--inter_label_weight', default=4.0, type=float, help='loss wright for label==1')
        parser.add_argument('--intra_label_weight', default=2.0, type=float, help='loss wright for label==1')
        parser.add_argument('--focusing', default=2, type=int, help='')
        parser.add_argument('--loss_mode', default='focal', help='focal/normal')
        parser.add_argument('--event_emb_mul', default=True, type=bool)
        parser.add_argument('--need_cls', default=True, type=bool)
        ...
        parser.add_argument('--layer_intra_loss_weight', default=[0.3, 3., 0., 0., 0.],
                            help='a list length = max graph + 1')
        parser.add_argument('--layer_inter_loss_weight', default=[0.,0., 0.1, 0.3, 1.],
                            help='a list length = max graph + 1')

    def _dataset_config():
        parser.add_argument('--maxlen', default=2048, type=int)
        parser.add_argument('--tokenlen', default=1900, type=int)
        parser.add_argument('--shiftlen', default=500, type=int)
    ...
    def _base_config():
        ...
        parser.add_argument('--encoder_device', default=-1, type=int)
        parser.add_argument('--run_mode', default='train',
                            help='train#/test#/resume#/stack#/stack#_test/stack#_resume')
        parser.add_argument('--notes', default='ctb-longformer',
    ...
    parser.add_argument('--train_epoch', default=64, type=int,
                        help='train epoch')

    # code backup
    parser.add_argument('--save_codes', default=["RRGCATlong_dataset.py",
                                                 "RRGCATlong_framework.py",
                                                 "RRGCATlong_model.py",
                                                 "RRGCATlong_option.py",
                                                 "RRGCATlong_graph_build.py"],
                        help='save code dir list')

    opt = parser.parse_args()

    # opt.stack_num, opt.version
    ...
    opt.encoder_device = opt.device if opt.encoder_device == -1 else torch.device(opt.encoder_device)
